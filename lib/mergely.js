	...
	diff: function(added, removed) {
		...
		for (var i = 0; i < changes.length; ++i) {
			...
			if (change.op != 'a') {
				...
				var rchange = d.getLines('rhs').slice(change['rhs-line-from'], change['rhs-line-to'] + 1).join(' ');
				// minimize span of the change from the left side
				var skip_left = 0;
				while (skip_left<Math.min(lchange.length, rchange.length) && lchange[skip_left] == rchange[skip_left])
					skip_left++;
				// minimize span of the change from the right side
				var skip_right = 0;
				while (lchange.length-skip_right > skip_left && rchange.length-skip_right > skip_left && lchange[lchange.length-1-skip_right] == rchange[rchange.length-1-skip_right])
					skip_right++;

				// output the changed index and text
				removed(li + skip_left, li + lchange.length - skip_right);
			}
			if (change.op != 'd') {
				// find the starting index of the line
				...
				var lchange = d.getLines('lhs').slice(change['lhs-line-from'], change['lhs-line-to'] + 1).join(' ');
				// minimize span of the change from the left side
				var skip_left = 0;
				while (skip_left<Math.min(lchange.length, rchange.length) && lchange[skip_left] == rchange[skip_left])
					skip_left++;
				...
				// output the changed index and text
				added(li + skip_left, li + rchange.length - skip_right);
			}
		}
	}
});
