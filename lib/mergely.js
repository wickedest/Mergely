...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	...
	bind: function(el) {
		...
		document.getElementById(this.id).addEventListener("keydown", function(evt) {
			...
			var shift_key=16, alt_key=17, ctrlKey=18, metaKey=91; // enterKey=13, escKey=27
			var up_arrow=38, down_arrow=40, left_arrow=37, right_arrow=39;
			var key_h_left=72, key_j_down=74, key_k_up=75, key_h_right=76;
			if (!evt.key) {
				// use keyCode as key
				evt.key = evt.which;
			}
			if (evt.shiftKey && evt.ctrlKey) {
				// VI convention : Ctrl + Shif + K/L/J/H
				if (evt.which==key_h_left)  evt.key = left_arrow;
				if (evt.which==key_j_down)  evt.key = down_arrow;
				if (evt.which==key_k_up)    evt.key = up_arrow;
				if (evt.which==key_h_right) evt.key = right_arrow;
			}
